...
/*   Updated: 2024/03/16 05:32:54 by ymomen           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_lst	*check_parss_erres(t_parse parc_line, t_lst **node, int *err)
{
	...
	while (tmp)
	{
		if((tmp ->prio > 0 && !tmp->next && tmp->type != CLOSE_PARENTH) || (tmp->next && ( tmp->prio > 2 && tmp->prio < 6 && tmp->next->prio > 2 && tmp->next->prio < 6))
			|| (tmp->next && tmp->next->prio == tmp->prio) || (tmp->next && tmp->type > 0  && tmp->next->type > 0 && tmp->next->type != OPEN_PARENTH  && tmp->next->type != 9 && tmp->next->type != 10 && tmp->type != 9 && tmp->type != 10 && tmp->next->type != 1 && tmp->type != 1 && tmp->next->type != 2 && tmp->type != 2)
			|| ((tmp->type == 1 || tmp->type == 2 || tmp->type == 9 || tmp->type == 10) && !(tmp->next->type < 0)) || (tmp->type <= 0 && tmp->next && tmp->next->type == OPEN_PARENTH))
		{
			*err = 258;
